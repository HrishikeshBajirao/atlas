export function CountryCardOutput({countryInfo, index, setCountries, setNumberOfSlots}){
    ...
    return (
        <div className="country-card w-full bg-slate-800 text-white px-[50px] py-5 rounded-lg border-1 relative ">
            <img src={countryInfo.flag.png} alt={`Flag of ${countryInfo.name}`} width="200" className="mx-auto rounded-lg" />
            <button type="button" 
            ...
            >X</button>
            <h2 id="card-title" className="text-center text-3xl my-3">{countryInfo.name}</h2>
              ...
              <div className="card-column grid gap-4">
                ...
                <p className="text-lg"><span className="font-semibold">📍 Capital: </span>{countryInfo.capital}</p>
                <p className="text-lg"><span className="font-semibold">🤝 Language: </span>{countryInfo.languages[0]}</p>
              </div>
              <div className="vertical-line hidden bg-gray-600 h-full md:block"></div>
              <div className="card-column grid gap-4">
                <p className="text-lg"><span className="font-semibold">🌐 Continent: </span>{countryInfo.region}</p>
                <p className="text-lg"><span className="font-semibold">🗺️ Area: </span>{countryInfo.area.toLocaleString()} sq km</p>
                <p className="text-lg"><span className="font-semibold">🪙 Currency: </span>{countryInfo.currency[0]}</p>
              </div>
            </div>
        </div>
    )
